...
const logs = [];
let stopSmokeWorker = false;
let smokeWorkerError;
let smokeWorker;
...
async function main() {
  try {
    ...
    stopSmokeWorker = true;
    if (smokeWorker) {
      await Promise.race([smokeWorker, sleep(1_000)]);
    }
    server.kill("SIGTERM");
  }
}
...
async function runSmokeWorker() {
  await workerPostJson("/api/codexdock/worker/connect", {
    workerId: "smoke-worker",
    deviceName: "smoke-worker",
    capabilities: [
      "generate_text",
      "generate_object",
      "generate_file",
  ...
  while (!stopSmokeWorker) {
    const response = await fetch(`${serverUrl}/api/codexdock/worker/next`, {
      method: "POST",
      headers: {
        ...workerHeaders(),
        "content-type": "application/json",
      },
      body: JSON.stringify({ workerId: "smoke-worker" }),
    });
    ...
    try {
      await workerPostJson("/api/codexdock/worker/result", {
        workerId: "smoke-worker",
        invocationId: invocation.invocationId,
        ok: true,
        result: buildSmokeResult(invocation),
      });
    } catch (error) {
      await workerPostJson("/api/codexdock/worker/result", {
        workerId: "smoke-worker",
...
async function workerPostJson(path, body) {
...
function buildSmokeResult(invocation) {
  const parameters = invocation.parameters ?? invocation.payload ?? {};
  ...
  if (invocation.type === "generate_text") {
    return {
      ...
      parameters,
      text: titleFromPrompt(invocation.prompt),
      provider: "codexdock",
      model: "local-codex",
    };
  }
    ...
    return {
      ...
      object: {
        items: Array.from({ length: count }, (_, index) => ({
          id: index + 1,
          title: `${titleFromPrompt(invocation.prompt)} ${index + 1}`,
          description: "Smoke worker output.",
        })),
      },
    };
  }
  ...
  if (invocation.type === "generate_file") {
    const filename = stringFromPayload(parameters, "targetPath", "README.md");
    return {
      kind: "file",
      ...
      encoding: "utf-8",
      content: `# ${titleFromPrompt(invocation.prompt)}\n\nGenerated by the CodexDock smoke worker.\n`,
    };
  }
  ...
  if (invocation.type === "generate_image") {
    ...
    return {
      ...
      promptUsed: invocation.prompt,
    };
  }

  throw new Error(`Unsupported smoke invocation type: ${invocation.type}`);
}
...
async function waitForWorker() {
  for (let attempt = 0; attempt < 60; attempt += 1) {
    if (smokeWorkerError) throw smokeWorkerError;
    const response = await fetch(`${serverUrl}/api/codexdock/worker/status`, {
      headers: workerHeaders(),
    });
    if (response.ok) {
