...
예제 앱은 외부 DB나 별도 서비스를 요구하지 않는 방향으로 만든다. Node.js 프로세스 메모리에 invocation queue를 두고, 개발용 internal smoke worker가 `worker/next`와 `worker/result` endpoint로 전체 흐름을 검증하는 바로 실행 가능한 샘플이면 충분하다. 실제 사용 경로의 기본 worker는 Codex SDK adapter다. 이 예제는 프로세스가 재시작되면 invocation 상태가 사라져도 된다.
...
- 개발용 smoke runner로 invoke 요청/응답 완료까지 검증
