...
CodexDock은 웹서비스가 OpenAI API를 직접 호출하지 않고, AI로 처리할 작업을 각 사용자의 로컬 Codex 실행 환경에 위임할 수 있게 해주는 SDK + CLI worker다.

핵심 목적은 다음과 같다.

- 웹서비스는 `@codexdock/sdk`를 설치해 AI 작업 요청을 생성한다.
- 로컬 사용자는 `codexdock` CLI를 실행해 웹서비스와 자신의 로컬 Codex를 연결한다.
- 사용자-facing AI 요청은 요청한 사용자의 owner scope에 저장되고, 그 사용자에게 연결된 worker가 처리한다.
- 서비스 공용 worker가 필요한 경우에는 별도 system owner로 명시적으로 연결한다.
...
`ownerId`는 브라우저 payload에서 그대로 받지 않는다. host app의 server-side auth/session에서 현재 사용자 또는 명시적인 system owner를 확인한 뒤 SDK 호출에 붙인다.
...
`pending`은 아직 worker가 claim하지 않은 상태를 뜻한다. 연결된 worker가 없어도 `invoke` 요청은 기본적으로 pending invocation을 생성한다. UI는 owner scope별 worker presence를 확인해 "내 로컬 Codex worker가 연결되어 있지 않음"을 안내한다.

초기 invoke type:

- `generate_object`: UI나 이미지 생성을 위한 structured object를 만든다.
- `generate_file`: 파일 내용 또는 파일 초안을 만든다.
- `generate_image`: 실제 이미지 artifact를 만든다. 결과는 `mediaType`과 `base64`를 포함해야 한다.
...
MVP에서는 worker가 `/api/codexdock/worker/result`에 위 artifact JSON을 inline으로 제출한다. 작은 텍스트 파일과 작은 이미지 검증에는 이 방식이 충분하다. 운영용 큰 파일/이미지는 host app별 storage adapter 또는 worker artifact upload endpoint를 추가해 binary를 storage에 올리고, 최종 result에는 `artifactId`, `url`, `storagePath` 같은 reference만 저장한다.

SDK는 completed result 저장 전에 invocation type별 result schema를 검증한다. `generate_file`은 file artifact, `generate_image`는 image artifact가 아니면 `INVALID_PAYLOAD`로 거부한다. 저장 직전에는 원본 invocation parameters를 result의 top-level `parameters`로 다시 붙여 host app이 후처리 용도를 판단할 수 있게 한다.
...
- 웹 UI에서 해당 owner의 사용자가 approve해야 worker token을 발급한다.
- pairing 결과로 발급되는 worker token은 host app, ownerId, ownerKind, workerId에 묶인다.

Worker auth:

- worker endpoint auth는 기본적으로 필수다.
- insecure worker auth는 local smoke test처럼 명시한 경우에만 허용한다.
- worker token은 특정 host app/owner/worker에 묶인다.
- 서버에는 token 원문을 저장하지 않고 hash만 저장한다.
- token revoke가 가능해야 한다.
- worker는 자신의 owner scope에 있는 invocation만 claim할 수 있다.
...
- 고정된 `/api/codexdock/*` path는 숨김 보안으로 취급하지 않는다.

Invoke safety:

- invoke endpoint는 host app의 사용자/관리자 인증, quota, rate limit으로 보호한다.
- host app은 현재 로그인 사용자 또는 명시적인 system owner를 기준으로 `ownerId`를 설정한다.
- 클라이언트가 보낸 ownerId를 그대로 신뢰하지 않는다.
...
- `workerToken` 없이 SDK를 생성하면 명시적인 insecure mode가 아닌 한 실패한다.
...
- `generate_object`가 structured object envelope을 반환한다.
- `generate_image`가 `kind: "image"`, `mediaType`, `encoding: "base64"`, `base64`를 포함한 artifact result를 반환한다.
